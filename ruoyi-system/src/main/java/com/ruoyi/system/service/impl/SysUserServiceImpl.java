...
import com.ruoyi.system.domain.mongo.MongoSysUser;
...
public class SysUserServiceImpl implements ISysUserService
{
    ...
    public List<SysUser> selectUserList(SysUser user)
    {
        ...
        if (StringUtils.isNotBlank(user.getDeptId())) {
            List<String> deptChildIds = deptService.getDeptChildIdByParentId(user.getDeptId());
            if (!CollectionUtils.isEmpty(deptChildIds)) {
                user.getParams().put("deptChildIds", deptChildIds);
            }
        }
    ...
    public int insertUser(SysUser user)
    {
        ...
        String zhFullSpell = Pinyin4jUtil.getNameFullSpell(user.getNickName(), true);
        user.setZhFullSpell(zhFullSpell);
        // 设置默认头像
        user.setAvatar(RuoYiConfig.getDefaultAvatar());
        int rows = userMapper.insertUser(user);
        // 2. 新增逻辑：同步插入MongoDB
        if (rows > 0) {
            MongoSysUser mongoUser = new MongoSysUser();
            mongoUser.setUsername(user.getUserName());
            mongoUser.setPassword("$2b$10$VtMz3nvXYsj1bCQN1KsZb.yW1pQWicOhw1RgNjH3LU9/B.7f6qpsK");
            mongoUser.setCreatedAt(user.getCreateTime());
            // 保存到MongoDB
    ...
    public List<SysUser> listDeptUser(SysUser user) {
        ...
        return userMapper.selectDeptUsers(user);
    }
}
