...
public class FlowTaskServiceImpl extends FlowServiceFactory implements IFlowTaskService {
    ...
    public void taskReject(FlowTaskVo flowTaskVo) {
        try {
            ...
            if (Objects.isNull(task)) {
                throw new FlowableHandleException("当前任务不存在或已被处理");
            }
            ...
            List<String> currentTaskIds = getCurrentUserTaskIds(
                    task,
                    parentUserTaskList.get(0),
                    FlowCommentEnum.REJECT,
                    flowTaskVo.getComment()
            );
            if (currentTaskIds.isEmpty()) {
                throw new FlowableHandleException("未找到当前需要处理的任务节点");
            }

            // 7. 获取流程实例中已处理的活动实例映射（用于恢复处理人）
            Map<String, List<ActivityInstance>> activityInstanceListMap = getActivityInstanceMapByProcInsId(flowTaskVo.getProcInsId());

            // 8. 执行流程节点跳转（根据目标节点数量处理不同场景）
            if (targetIds.size() == 1) {
                // 场景1：单目标节点（1对1或多对1）
...
//    }
    ...
    public Boolean checkRejectCondition(FlowTaskVo flowTaskVo) {
        ...
        if (!isAssignee) {
            // 检查是否为候选人
            List<IdentityLink> candidateLinks = taskService.getIdentityLinksForTask(taskId);
            boolean isCandidate = candidateLinks.stream()
...
//        if (!isAssignee) {
//            // 若不是直接负责人，检查是否在候选人中
//            List<String> candidates = taskService.getIdentityLinksForTask(taskId).stream()
...
//    }
     ...
     */
        ...
        // 仅对当前任务添加一次评论（当前操作用户的意见）
        if (comment != null) {
            taskService.addComment(
                    task.getId(),  // 仅关联当前操作的任务ID
                    task.getProcessInstanceId(),
                    flowCommentEnum.getType(),
                    comment  // 仅添加当前用户输入的意见
            );
            log.info("添加退回意见成功，taskId: {}，意见：{}", task.getId(), comment);
        }

        // 仅返回当前任务的ID（避免处理其他任务）
        return Collections.singletonList(task.getTaskDefinitionKey());
