...
//    @Autowired
//    private ImChatMessageMapper holidaySettingMapper;
    ...
    public void process123(ChannelHandlerContext ctx, Claims claims) throws IOException, InterruptedException {
        String token = (String) claims.get(LOGIN_USER_KEY);
        String tokenKey = getTokenKey(token);
        LoginUser loginUser = redisCache.getCacheObject(tokenKey);
        String userId = loginUser.getUser().getUserId();
        ...
        redisLock.doLock(SOCKET_USER_LOCK_KEY + userId, 10, TimeUnit.SECONDS, () -> {
            ChannelHandlerContext context = UserChannelCtxMap.getChannelCtx(userId, userId);
            if (context != null && !ctx.channel().id().equals(context.channel().id())) {
                // 不允许多地登录,强制下线
                SendInfo<Object> sendInfo = new SendInfo<>();
                sendInfo.setCmd(CmdTypeEnum.FORCE_LOGOUT.code());
...
//            List<ChatSessionVo> listUser= holidaySettingMapper.selectChatSessions(userId);
//            for (ChatSessionVo chat:listUser) {
//                // 绑定用户和channel 修改可能恢复
//                UserChannelCtxMap.addChannelCtx(userId, chat.getId(), ctx);
              //  UserChannelCtxMap.addChannelCtx(userId, userId, ctx);
            ...
            UserChannelCtxMap.addChannelCtx(userId, userId, ctx);
                ...
                // 设置用户token
                AttributeKey<String> tokenAttr = AttributeKey.valueOf(ChannelAttrKey.USER_TOKEN);
                ctx.channel().attr(tokenAttr).set(userId);
                // 初始化心跳次数
                AttributeKey<Long> heartBeatAttr = AttributeKey.valueOf(ChannelAttrKey.HEARTBEAT_TIMES);
                ctx.channel().attr(heartBeatAttr).set(2L);
                // 在redis上记录每个user的channelId，15秒没有心跳，则自动过期
                String key = String.join(":", SocketRedisKey.IM_USER_SERVER_ID, userId, userId);
                redisExtendTemplate.opsForValue().set(key, SocketServerGroup.serverId, SocketConstant.ONLINE_TIMEOUT_SECOND, TimeUnit.SECONDS);
