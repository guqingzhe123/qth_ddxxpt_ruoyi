...
public class ScheduleServiceImpl implements IScheduleService {
    ...
    @Autowired
    private TodoMapper todoMapper;//协同办公
    ...
    public void Push() {
       //查询业务是否办理
        List<Todo> todos = todoMapper.selectTodoList(new Todo());
        String message = "您有新的待办事项，请尽快处理。";
        for (Todo todo:todos ) {
            List<String> recvIds=new ArrayList<>();
            recvIds.add(todo.getCurHandler());
            if(recvIds.size()>0){
                systemMessageService.sendBusinessSystemMessage(com.ruoyi.message.enums.BusinessMessageType.TODO_RECEIVE.getCode(),
                        todo.getSender(),recvIds,message);
            }
//            log.info("## 推送发送完成！");
        }
        //todoMapper.listTodo(todo)
    }
