...
public class TokenService {
     ...
     * 删除在线用户token
     */
    public void delOnlineUser(LoginUser loginUser) {
        if (StringUtils.isNotEmpty(loginUser.getToken())) {
            String userKey = CacheConstants.USER_TOKEN_KEY + loginUser.getUserId();
            redisCache.deleteObject(userKey);
        }
    }
    ...
    public String createToken(LoginUser loginUser) {
        // 检查用户是否已经有登录的token，如果有则删除旧的token
    ...
    private String getToken(HttpServletRequest request) {
        String token = StringUtils.isBlank(request.getHeader(header)) ? request.getParameter(header) : request.getHeader(header);
        // 此处兼容ureport的token问题，从cookie中获取，如未使用，可注释此代码（如果用户禁用了cookie，会导致使用ureport报表设计或编辑时，保存报错，但不影响用户正常查看已编辑好的报表）
        if (StringUtils.isBlank(token) && request.getRequestURI().contains("ureport")) {
            Cookie[] cookies = request.getCookies();
            if (cookies != null) {
                for (Cookie cookie : cookies) {
                    if (cookie.getName().equals("Admin-Token")) {
                        token = Constants.TOKEN_PREFIX + cookie.getValue();
                        break;
                    }
                }
            }
