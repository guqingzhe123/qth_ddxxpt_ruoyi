...
public class KbsTopicCategoryServiceImpl implements IKbsTopicCategoryService {
    ...
    public int deleteKbsTopicCategoryByIds(String[] ids) {
        try {
            ...
            KbsTopicInfo info=new KbsTopicInfo();
            info.setCategoryId(ids[0]);
            List<KbsTopicInfo> kbsTopicInfos = kbsTopicInfoMapper.selectKbsTopicInfoList(info);
            // 核心转换代码
            String[] idss = kbsTopicInfos.stream().map(kbsTopicInfo -> String.valueOf(kbsTopicInfo.getId())).toArray(String[]::new);
            // 收集成 String 数组
            kbsTopicInfoMapper.deleteKbsTopicInfoByIds(idss);
