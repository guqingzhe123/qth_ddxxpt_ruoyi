...
public abstract class AbstractBizFlowSubmitData implements IBizFLowSubmitService {
    ...
    public void submit(CommonFlowSubmit submit) {
        ...
        saveAttachments(submit.getFlowTask()); // 保存附件关联
        afterSubmit(submit);
...
private void handleBizFlowData(CommonFlowSubmit submit) {
    log.debug("处理流程数据 {}", JSONUtil.toJsonStr(submit));
    if (submit == null || submit.getFlowTask() == null) {
        return;
    }
    if (StringUtils.isNotBlank(submit.getOperateType())) {
        ...
        if (operateTypeEnum == null) {
            if ("300".equals(submit.getOperateType())){
                submit.getFlowTask().setOperateType(OperateTypeEnum.COMPLETE.getType());
            }
            operateTypeEnum = OperateTypeEnum.getByType(submit.getFlowTask().getOperateType());
            if(operateTypeEnum == null){
                return;
            }
        }
    ...
    // 在submitFlowTaskBySync方法后添加保存附件的方法
    private void saveAttachments(FlowTaskVo flowTaskVo) {
        // 保存附件关联（如果有）
        if (CollectionUtil.isNotEmpty(flowTaskVo.getFileIds()) && StringUtils.isNotBlank(flowTaskVo.getProcInsId())) {
            for (String fileId : flowTaskVo.getFileIds()) {
                FlowApprovalAttachment attachment = new FlowApprovalAttachment();
                attachment.setId(IdUtils.fastSimpleUUID());
                attachment.setProcInsId(flowTaskVo.getProcInsId());
                attachment.setFileId(fileId);
                attachment.setCreateBy(flowTaskVo.getTaskId());
