            ...
            while (!uploadSuccess && retryCount < 3) {
                try {
                    ...
                    if (e.getMessage().contains("NoSuchUpload")) {
                        log.warn("上传ID失效，重新创建多部分上传: {}", e.getMessage());
                        // 删除失效的缓存，重新创建上传会话
                        redisCache.deleteObject(cacheKey);
                        uploadFileInfo = createNewMultipartUpload(fileUrl, headers, cacheKey);
                        retryCount++;
                        inputStream = multipartFileParam.getUploadInputStream(); // 重新获取输入流
                    } else {
                        throw e; // 非NoSuchUpload错误直接抛出
                    }
                }
            }

            if (!uploadSuccess) {
                throw new BaseException("分片上传失败，重试次数已达上限");
            }
        } catch (Exception e) {
            log.error("分片上传失败", e);
...
//    }
    ...
    private void completeMultipartUpload(UploadFile uploadFile) {
        ...
        try {
            ...
            if (e.getMessage().contains("NoSuchUpload")) {
                log.error("上传ID已失效，无法完成合并: {}", e.getMessage());
                // 清理失效缓存
                redisCache.deleteObject(cacheKey);
