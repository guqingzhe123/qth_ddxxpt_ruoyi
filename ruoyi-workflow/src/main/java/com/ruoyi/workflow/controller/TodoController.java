...
public class TodoController extends BaseController {
    ...
    public TableDataInfo<BaseEntity> list(Todo todo) {
        startPage();
     ...
     * 查询待办列表
     */
    @GetMapping("/list/tabledraft")
    public TableDataInfo<BaseEntity> tabledraft(Todo todo) {
        startPage();
        List<Todo> list = todoService.listTodoDraft(todo);
        return getDataTable(list);
    }
    /**
     * 查询待办列表（折叠，不分页）
