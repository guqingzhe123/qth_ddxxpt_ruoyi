...
public class FlowHandleServiceImpl implements IFlowHandleService {
    ...
    @Autowired
    private IBusinessSystemMessageService businessSystemMessageService;
    ...
    public void completeTask(FlowTaskVo flowTaskVo, String createId) {
        if(flowTaskVo.getOperateType()==null){
            log.error("[流程id为空] 传过来参数"+flowTaskVo);
            return;
        }
        ...
        //查询是否所有审批都同意
        int num= todoService.selectLastTodo(flowTaskVo.getBusinessId());
        //查询下一个环节
        if(flowTaskVo.getOperateType().equals("2") && num==0 && (nextTodos == null || nextTodos.isEmpty())){
            // 改变状态
            handleSetHandleType(flowTaskVo,"0","3");//handleType
            // 处理其他操作  除了发送通知
            handleOtherOperation(nextTodos, createId, false,"任务已完成！");
        }
        if(nextTodos.size()>0){
            List<String> recvIds=new ArrayList<>();
            String senderName="";
            for (Todo todo : nextTodos) {
                recvIds.add(todo.getCurHandler());
                senderName=todo.getSenderName();
            }
//            String message = String.format(
//                    "[%s] 您有新的待办事项，请尽快处理。",
//                    senderName
//            );
            String message = "您有新的待办事项，请尽快处理。";
            systemMessageService.sendBusinessSystemMessage(BusinessMessageType.TODO_RECEIVE.getCode(), createId, recvIds,message);
        }else {
            List<String> recvIds=new ArrayList<>();
            WorkflowMyDraft workflowMyDraft = workflowMyDraftService.updateHandleTypeByProcInsId(flowTaskVo.getBusinessId());
            recvIds.add(workflowMyDraft.getCreateId());
            SysUser targetUser = sysUserService.selectUserById(createId);
            if (targetUser == null) {
                log.error("催办通知发送失败：目标用户不存在，用户ID：{}", flowTaskVo.getTemplateType());
                return;
            }
            String message = String.format("[%s] 已办理了您的协同事项。",targetUser.getNickName());
            // 处理其他操作  除了发送通知
            handleOtherOperation(nextTodos, createId, false,message);
            systemMessageService.sendBusinessSystemMessage(BusinessMessageType.TODO_RECEIVE.getCode(), createId,recvIds,message);
        }
        // 给待办用户通知
        //flowHandleService.sendUrgeNotification(flowTaskVo);
    }
    ...
    /**
     * 给待办用户通知
     * @param flowTaskVo 流程任务对象
     */
    // 修改 FlowHandleServiceImpl 中的 sendUrgeNotification 方法
    @Override
    public void sendUrgeNotification(FlowTaskVo flowTaskVo) {

        if (StringUtils.isBlank(flowTaskVo.getTaskId())) {
            log.error("催办通知发送失败：任务ID为空");
            return;
        }

        // 1. 校验前端传递的“被催办用户ID”是否存在
        String targetUserId = flowTaskVo.getUserId();
        if (StringUtils.isBlank(targetUserId)) {
            log.error("催办通知发送失败：未指定被催办用户（urgeUserId为空）");
            return;
        }

        try {
            // 2. 验证目标用户是否存在（避免无效用户ID）
            SysUser targetUser = sysUserService.selectUserById(targetUserId);
            if (targetUser == null) {
                log.error("催办通知发送失败：目标用户不存在，用户ID：{}", targetUserId);
                return;
            }
            String senderId = SecurityUtils.getUserId();
            // 3. 获取流程实例信息（用于消息内容）
            // （从待办列表中获取流程实例ID，仅为了获取流程名称，不用于接收者）
            List<Todo> todoList = todoService.listTodoByTaskId(flowTaskVo.getTaskId());
            if (CollectionUtils.isEmpty(todoList)) {
                log.warn("未找到任务对应的待办信息，任务ID：{}", flowTaskVo.getTaskId());
                return; // 或继续发送（仅缺少流程名称）
            }
            String procInstId = todoList.get(0).getProcInstId();


//            HistoricProcessInstance historicProcessInstance = flowInstanceService.getHistoricProcessInstanceById(procInstId);
//            String flowName = historicProcessInstance != null
...
//                    : "未知流程"; // 兜底处理
            ...
            if (senderUser == null) {
                log.error("催办通知发送失败：当前登录用户不存在");
                return;
            }
            String senderName = StringUtils.isNotBlank(senderUser.getNickName())
                    ? senderUser.getNickName()
                    : senderUser.getUserName();

            // 5. 构建接收者列表（仅包含选择的目标用户）
            List<String> recvIds = new ArrayList<>();

            // 6. 构建通知内容（明确是“发送者”催促“目标用户”）
...
//            for (Object value : flowTaskVo.getUserId().values()) {
//                // 处理 null 值（避免调用 toString() 时抛空指针）
//                String strValue = (value != null) ? value.toString() : null;
//                recvIds.add(strValue);
//            }

            recvIds.add(flowTaskVo.getUserId());

            // 7. 发送系统通知（发送者：当前用户，接收者：选择的目标用户）
            businessSystemMessageService.sendBusinessSystemMessage(
                    BusinessMessageType.TODO_RECEIVE.getCode(),  // 消息类型
                    senderId,                                   // 发送者ID（当前用户）
                    recvIds,                                    // 接收者ID（选择的用户）
                    msgContent                                   // 消息内容
            );
...
//                    .templateId("URGE_NOTICE_TEMPLATE")
//                    .build();
//            syncPush.push(
//                    QueueEnum.ASYNC_MESSAGE_QUEUE.getConsumerBeanName(),
//                    JSON.toJSONString(messageNotice),
//                    QueueEnum.ASYNC_MESSAGE_QUEUE.getQueueName(),
//                    senderId
//            );

            log.info("催办通知发送成功，任务ID：{}，发送者：{}，接收者（选择的用户）：{}",
                    flowTaskVo.getTaskId(), senderId, recvIds);
        } catch (Exception e) {
            log.error("发送催办通知失败", e);
        }
    }
    ...
    public void revokeProcess(FlowTaskVo flowTaskVo) {
        ...
        handleNextTodos(nextTodos, flowTaskVo);
        // 处理其他操作
        handleOtherOperation(nextTodos, SecurityUtils.getUserId(), true,"您的待办任务已更新！");
    }
    ...
    public void taskReject(FlowTaskVo flowTaskVo, String createId) {
        ...
        // 改变状态
         handleSetHandleType(flowTaskVo,"1","3");
        List<String> recvIdsd=new ArrayList<>();
        WorkflowMyDraft workflowMyDraft = workflowMyDraftService.updateHandleTypeByProcInsId(flowTaskVo.getBusinessId());
        recvIdsd.add(workflowMyDraft.getCreateId());
        ...
        handleOtherOperation(nextTodos, createId, false,message);
        // 给待办用户通知
        systemMessageService.sendBusinessSystemMessage(BusinessMessageType.TODO_RECEIVE.getCode(), createId, recvIdsd,message);
     ...
     * 异步处理我起草的流程
     */
    private void handleSetHandleType(FlowTaskVo flowTaskVo,String handleType,String Status) {
        WorkflowMyDraft workflowMyDraft=new WorkflowMyDraft();
        workflowMyDraft.setBizId(flowTaskVo.getBusinessId());
        workflowMyDraft.setHandleType(handleType);
        workflowMyDraft.setStatus(Status);
        workflowMyDraftService.updateHandleTypeByProcInsId(workflowMyDraft);
    ...
    public void copyTask(FlowTaskVo flowTaskVo) {
        ...
        // 处理其他操作
        handleOtherOperation(copyTaskTodos, SecurityUtils.getUserId(), true,"您的待办任务已更新！");
    }
    ...
    public void readCopyTodo(String id) {
        ...
        handleOtherOperation(null, SecurityUtils.getUserId(), false,"您的待办任务已更新！");
    }
    ...
    public void taskReturn(FlowTaskVo flowTaskVo, String createId) {
        ...
        handleNextTodos(nextTodos, flowTaskVo);
        // 处理其他操作
        handleOtherOperation(nextTodos, createId, true,"您的待办任务已更新！");
    }
    ...
    public Boolean checkCompleteCondition(FlowTaskVo flowTaskVo) {
        boolean bool=flowTaskService.checkCompleteCondition(flowTaskVo);
//        if(bool) {
//            //发送推送
//            flowHandleService.sendUrgeNotificationUser(flowTaskVo);
//        }
        return bool;
    }
    ...
    public Boolean checkRejectCondition(FlowTaskVo flowTaskVo) {
       boolean bool=  flowTaskService.checkRejectCondition(flowTaskVo);
//       if(bool) {
//           //发送推送
//           flowHandleService.sendUrgeNotificationUser(flowTaskVo);
//       }
     ...
     * @param message 弹窗消息
     */
    private void handleOtherOperation(List<Todo> nextTodos, String createId, Boolean isMessageNotice,String message) {
        try {
    ...
    private void deleteClaimOtherTodo(FlowTaskVo flowTaskVo, String createId) {
        ...
        // 处理其他操作
        handleOtherOperation(filterTodo, createId, false,"您的待办任务已更新！");
    }
