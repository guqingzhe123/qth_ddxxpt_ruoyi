...
public class TodoServiceImpl implements ITodoService {
     ...
     */
    @Override
    public List<Todo> listTodoDraft(Todo todo) {
        todo.setDelFlag(WhetherStatus.NO.getCode());
        todo.setCurHandler(SecurityUtils.getUserId());
        return todoMapper.selectTodoListDraft(todo);
    }
    /**
     * 查询待办列表（折叠）
     *
    ...
    public List<TodoCollapseResult> listCollapse(Todo todo) {
        ...
        // 过滤掉 templateType 为 null 的数据
        List<Todo> filteredList = list.stream()
                .filter(t -> t.getTemplateType() != null)
                .collect(Collectors.toList());

        // 按模板分类分组（此时分组键不会为 null）
        List<String> templateTypes = filteredList.stream()
                .map(Todo::getTemplateType)
                .distinct()
                .collect(Collectors.toList());
        List<TemplateType> templateTypList = templateTypeService.listTemplateType(templateTypes);
        Map<String, String> templateMap = templateTypList.stream()
                .collect(Collectors.toMap(TemplateType::getId, TemplateType::getName));
        Map<String, List<Todo>> map = filteredList.stream()
                .collect(Collectors.groupingBy(Todo::getTemplateType));

        return map.entrySet().stream().map(entry -> {
            TodoCollapseResult result = new TodoCollapseResult();
            String templateTypeId = entry.getKey();
            result.setTemplateId(templateTypeId);
...
//    }
     ...
     * 批量删除待办
     *
     * @param businessId 查询还有多少未处理
     * @return 结果
     */
    @Override
    public int selectLastTodo(String businessId) {
        return todoMapper.selectLastTodo(businessId);
    }
}
